...
export default class DeviceHelper {
    ...
    static async getDeviceById(id) {
        const databaseModel = new DatabaseModel();
        const query = `SELECT d.*, GROUP_CONCAT(n.number_id SEPARATOR ', ') AS linked_msisdns FROM devices d LEFT JOIN device_number n ON d.id = n.device_id WHERE d.id = ${id} GROUP BY d.id`;
    ...
    static async setOrUpdateMsisdnLink(deviceId, msisdnId) {
        const databaseModel = new DatabaseModel();
        const query = `INSERT INTO device_number (device_id, number_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE device_id = ?, number_id = ?`;
    ...
    static async deleteInactiveMsisdnLinks(deviceId, msisdns = []) {
        const databaseModel = new DatabaseModel();
        let query = `DELETE FROM device_number WHERE device_id = ?`;
